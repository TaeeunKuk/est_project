    ...
    const cats = Array.from(
      new Map(
        todos
          ...
          .map(t => [t.categoryId, { id: t.categoryId, name: t.categoryName ?? '카테고리' }])
      ).values()
    );
    if (cats.length) setCategories(prev => {
  ...
  return (
    <div className="category-manager">
      ...
      <div className="cat-input-group">
        <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="새 카테고리 추가" />
        <button className="btn-primary" onClick={addCategory}>추가</button>
      </div>
      ...
      <ul className="cat-list">
        {categories.map(cat => (
          <li key={cat.id} className="cat-chip">
            ...
            <span>{cat.name}</span>
            <button className="btn-del-cat" onClick={() => deleteCategory(cat.id)}>×</button>
          </li>
        ))}
      </ul>
    </div>
